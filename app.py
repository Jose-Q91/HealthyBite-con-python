...
app = Flask(__name__)
app.secret_key = (
    "tu_clave_secreta"  # Asegúrate de cambiar esto en un entorno de producción
)
...
def main():
    ...
    if "id_user" in session:
        ...
        mylikes = [like[1] for like in mylikes]
        categorys = [cat for cat in categorys if cat[1] in mylikes]
...
def filter_bycategory():
    if request.method == "POST":
        ...
        return render_template(
            "view_product.html", products=products, id_category=id_category
        )
    categorys = cnx.category_findall()
