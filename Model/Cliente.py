  ...
  def menu(self):
    ...
    while True:
      ...
      if opcion == '1':
        ...
        list_products = self.cnx.filter_product_ByIdCategory(id_category)
        ...
        product = self.cnx.product_findByID(id_product)
        ord = self.cnx.filter_orderByIDClient()
        try:
          detail = Detail(None, product[5], quantity, datetime.date.today(), ord[0], product[0])
          self.cnx.insert_detail(detail)
