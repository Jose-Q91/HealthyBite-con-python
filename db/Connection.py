...
class ConnectionSQLite(object):
    ...
    def __new__(cls):
        if not cls._instance:
            ...
            cls._instance.connection = sqlite3.connect(
                "restaurante.db", check_same_thread=False
            )
    ...
    def login(self, user, passw):
        user_data = self.one_fetch_data(
            """SELECT * FROM user WHERE username = ? AND password = ?""", (user, passw)
        )
        return user_data
    ...
    def filter_orderByIDClient(self):
        return self.one_fetch_data(
            "SELECT * FROM orders WHERE id_user = ? and date = ?",
            (session["id_user"], datetime.date.today()),
        )
    ...
    def edit_stock_product(self, stock, id):
        self.execute_query(
            "UPDATE product SET stock = ? WHERE id_product = ?", (stock, id)
        )
    ...
    def delete_like(self, mylike):
        return self.execute_query(
            "DELETE FROM mylikes WHERE name = ? and id_user = ?",
            (
                mylike,
                session["id_user"],
            ),
    ...
    def get_all_likes(self):
        return self.fetch_data(
            "SELECT * FROM mylikes WHERE id_user = ?", (session["id_user"],)
        )

    # COMPRA
    def get_buies(self):
        return self.fetch_data(
            "SELECT product.name, deatail.price, deatail.quantity, orders.price_tot from orders inner join deatail on orders.id_ord =  deatail.id_ord inner join product on deatail.id_product = product.id_product where orders.id_user = ? and orders.date = ?",
            (session["id_user"], datetime.date.today()),
        )
